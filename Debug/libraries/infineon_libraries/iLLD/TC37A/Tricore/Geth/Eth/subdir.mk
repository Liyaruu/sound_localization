...
libraries/infineon_libraries/iLLD/TC37A/Tricore/Geth/Eth/%.src: ../libraries/infineon_libraries/iLLD/TC37A/Tricore/Geth/Eth/%.c libraries/infineon_libraries/iLLD/TC37A/Tricore/Geth/Eth/subdir.mk
	...
	if [ -f "$(basename $@).d" ]; then sed.exe -r  -e 's/\b(.+\.o)\b/libraries\/infineon_libraries\/iLLD\/TC37A\/Tricore\/Geth\/Eth\/\1/g' -e 's/\\/\//g' -e 's/\/\//\//g' -e 's/"//g' -e 's/([a-zA-Z]:\/)/\L\1/g' -e 's/\d32:/@TARGET_DELIMITER@/g; s/\\\d32/@ESCAPED_SPACE@/g; s/\d32/\\\d32/g; s/@ESCAPED_SPACE@/\\\d32/g; s/@TARGET_DELIMITER@/\d32:/g' "$(basename $@).d" > "$(basename $@).d_sed" && cp "$(basename $@).d_sed" "$(basename $@).d" && rm -f "$(basename $@).d_sed" 2>/dev/null; else echo 'No dependency file to process';fi
...
clean: clean-libraries-2f-infineon_libraries-2f-iLLD-2f-TC37A-2f-Tricore-2f-Geth-2f-Eth

clean-libraries-2f-infineon_libraries-2f-iLLD-2f-TC37A-2f-Tricore-2f-Geth-2f-Eth:
	-$(RM) ./libraries/infineon_libraries/iLLD/TC37A/Tricore/Geth/Eth/IfxGeth_Eth.d ./libraries/infineon_libraries/iLLD/TC37A/Tricore/Geth/Eth/IfxGeth_Eth.o ./libraries/infineon_libraries/iLLD/TC37A/Tricore/Geth/Eth/IfxGeth_Eth.src

.PHONY: clean-libraries-2f-infineon_libraries-2f-iLLD-2f-TC37A-2f-Tricore-2f-Geth-2f-Eth

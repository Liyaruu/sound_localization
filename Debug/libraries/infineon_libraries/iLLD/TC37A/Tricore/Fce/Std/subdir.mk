...
libraries/infineon_libraries/iLLD/TC37A/Tricore/Fce/Std/%.src: ../libraries/infineon_libraries/iLLD/TC37A/Tricore/Fce/Std/%.c libraries/infineon_libraries/iLLD/TC37A/Tricore/Fce/Std/subdir.mk
	...
	if [ -f "$(basename $@).d" ]; then sed.exe -r  -e 's/\b(.+\.o)\b/libraries\/infineon_libraries\/iLLD\/TC37A\/Tricore\/Fce\/Std\/\1/g' -e 's/\\/\//g' -e 's/\/\//\//g' -e 's/"//g' -e 's/([a-zA-Z]:\/)/\L\1/g' -e 's/\d32:/@TARGET_DELIMITER@/g; s/\\\d32/@ESCAPED_SPACE@/g; s/\d32/\\\d32/g; s/@ESCAPED_SPACE@/\\\d32/g; s/@TARGET_DELIMITER@/\d32:/g' "$(basename $@).d" > "$(basename $@).d_sed" && cp "$(basename $@).d_sed" "$(basename $@).d" && rm -f "$(basename $@).d_sed" 2>/dev/null; else echo 'No dependency file to process';fi
...
clean: clean-libraries-2f-infineon_libraries-2f-iLLD-2f-TC37A-2f-Tricore-2f-Fce-2f-Std

clean-libraries-2f-infineon_libraries-2f-iLLD-2f-TC37A-2f-Tricore-2f-Fce-2f-Std:
	-$(RM) ./libraries/infineon_libraries/iLLD/TC37A/Tricore/Fce/Std/IfxFce.d ./libraries/infineon_libraries/iLLD/TC37A/Tricore/Fce/Std/IfxFce.o ./libraries/infineon_libraries/iLLD/TC37A/Tricore/Fce/Std/IfxFce.src

.PHONY: clean-libraries-2f-infineon_libraries-2f-iLLD-2f-TC37A-2f-Tricore-2f-Fce-2f-Std
